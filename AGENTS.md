...
WorkflowProgram 是一个面向 Codex 风格工作区的元工作流仓库。
...
- `.Codex/skills/develop/spec-template.md`
  `/develop` 生成 `workflow-spec.md` 时使用的规格模板。

## Development Commands

- Run：在 Codex 或 Codex 中打开本仓库后直接调用上述命令
- Test：`powershell -ExecutionPolicy Bypass -File .Codex/scripts/validate-workflow.ps1`
- Lint：`powershell -ExecutionPolicy Bypass -File .Codex/scripts/validate-workflow.ps1`
- Build：`N/A`

## 规则

- NEVER 直接在 `main` 分支上提交。
- ALWAYS 保持 `.Codex/settings.json` 与实际对外命令、技能同步。
- ALWAYS 保持 `README.md`、`lessons.md` 和 `.Codex/rules/constraints.md` 存在。
...
- 测试命令：`powershell -ExecutionPolicy Bypass -File .Codex/scripts/validate-workflow.ps1`
- 共享工作流变更在交付前必须通过全部仓库校验。
- 新增用户可见命令或技能时，必须同步更新 `README.md`、`AGENTS.md` 和 `.Codex/settings.json`。
- 可复用的长期约束应写入 `.Codex/rules/constraints.md`。
